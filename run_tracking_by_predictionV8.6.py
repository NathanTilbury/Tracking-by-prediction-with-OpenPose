...
'''
To Run:
python Tracking_by_predictionV8.9 --model=cmu --resize=656x368 --camera=0

or

python Tracking_by_predictionV8.9 --model=cmu --resize=656x368 --video=motion_test_vid.mp4
'''
import argparse
import time
import os
...
def get_eclid_dist(LTP, locate):
    ...
    '''
    print("\npredicted POS array ", predicted_positions)
    tracked = []
    highval = [sys.float_info.max, sys.float_info.max]
    print("eclid, locations", locate)
    ...
    for i, loc in enumerate(locate):
        ...
        eclidian_dist = sqrt( (p1[0]-p2[0])**2 + (p1[1]-p2[1])**2 )
        print("eclid dist ", eclidian_dist)
        if(eclidian_dist < highval[0]):
            highval[0] = eclidian_dist
            tracked = loc
...
def run_multi_corrector(tracked, LTP, locate):
    ...
    print("\ncurrent tracked", tracked, "\ttemp tracked ", temp_tracked)
...
def run_commands(humans):
    ...
    if need_user:
        ...
        if(leftHand.y < MID_LOWERl and leftHand.y > MID_UPPER and rightHand.y > GROUND): #
            ...
            # Both hands are on hips and ebows out
...
#   Variable used to display the FPS - this is for testing purposes.
    ...
    while(camera.isOpened()):
        ...
        if(ret == True):  
            print("The need user at the start of loop is: {0} \tThe ID of the user is {1}".format(need_user, tracked_human))
            try:
            ...
            print("The need user at the end of loop is: {0} \tThe ID of the user is {1}".format(need_user, tracked_human))

            if(need_user):
                pass
            else:
                print("\n Loop start: ", locations)
        ...
        #capturing the number of frames per second and writing this out to the sceen.
...
if __name__ == '__main__':
    ...
    '''
    parser = argparse.ArgumentParser(description='tf-pose-estimation realtime guesture recognition and tracking')
    ...
    parser.add_argument('--resize', type=str, default='0x0',
                        help='if provided, resize images before they are processed. default=0x0. I recomend 432x368 or 656x368 or 1312x736 ')
